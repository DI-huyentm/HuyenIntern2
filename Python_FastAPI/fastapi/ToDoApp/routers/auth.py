...
async def change_password(request: Request, username: str, password: str, new_password: str, db: Session = Depends(get_db)):
    user = db.query(models.Users).filter(models.Users.username == username).first()
    if user is None or not bcrypt_context.verify(password, user.get('hashed_password')):
        raise HTTPException(status_code=422, detail="Username or Current Password is incorrect")
...
#Exceptions: usually for using restful API only
